...
Both PDFs are generated with realistic Italian bureaucratic labels (setAlternateFieldName) and can be freely edited using any PDF form viewer. The forms are reproducibly generated by `SampleFormGenerator.java` (in `app/backend/src/test/.../tools/`) via Apache PDFBox.
